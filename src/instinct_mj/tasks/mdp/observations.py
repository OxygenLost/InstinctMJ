...
def _resolve_raycast_pattern(sensor: RayCastSensor) -> tuple[int, int, float]:
    ...
    if not isinstance(pattern, PinholeCameraPatternCfg):
        raise TypeError(
            f"Perceptive raycast depth only supports PinholeCameraPatternCfg. Got: {type(pattern).__name__}"
        )
...
def perceptive_depth_image(
    ...
    if isinstance(sensor, RayCastSensor):
        depth = _depth_from_raycast_distance_to_image_plane(sensor, depth_clipping_behavior="max")
    ...
    raise TypeError(f"Unsupported sensor type for perceptive_depth_image: {type(sensor).__name__}")
...
class _PerceptiveRaycastNoisedBase(ManagerTermBase):
    ...
    def __init__(self, cfg, env):
        ...
        self._frame_initialized = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        self._frame_step_count = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
    ...
    def _sample_depth_image(self, env) -> torch.Tensor:
        ...
        if self.update_period_steps <= 1:
            ...
            refresh = refresh | (self._frame_step_count.remainder(self.update_period_steps) == 0)
...
class PerceptiveRaycastNoisedHistory(_PerceptiveRaycastNoisedBase):
    ...
    def __call__(
        ...
        if uninitialized_env_ids.numel() > 0:
            repeated = frame[uninitialized_env_ids].unsqueeze(1).repeat(1, self.history_length, 1, 1)
